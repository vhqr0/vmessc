...
async def socks5_accept(buf: bytes, reader: StreamReader,
                        writer: StreamWriter) -> Tuple[str, int, bytes]:
    nmeths = buf[1]
    ...
    if atype == Socks5Atype.Domain:
        ...
        ver, cmd, rsv, atype, addr_bytes, port = struct.unpack(
            '!BBBB16sH', buf)
...
async def http_accept(buf: bytes, reader: StreamReader,
                      writer: StreamWriter) -> Tuple[str, int, bytes]:
    pos = buf.find(b'\r\n\r\n')
    if pos < 0:
    ...
    if method == 'CONNECT':
        ...
        rest = ('\r\n'.join(header for header in headers.split('\r\n')
                            if not header.startswith('Proxy-')).encode() +
                b'\r\n' + rest)
...
async def raw_connect(reader: StreamReader, writer: StreamWriter, addr: str,
                      port: int, rest: bytes):
